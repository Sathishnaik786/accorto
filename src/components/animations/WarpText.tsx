...
const getFontValue = (value: string | number | undefined) =>
...
interface BuildTextCanvasParams {
...
const buildTextCanvas = ({ container, width, height, dpr, props }: BuildTextCanvasParams) => {
...
interface ProgramWithUniforms {
  uniforms: {
    ...
    uSpeed: { value: number };
    uPointerInfluence: { value: number };
...
const syncUniforms = (program: ProgramWithUniforms, props: WarpTextProps) => {
  const uniforms = program.uniforms;
  if (props.warpStrength !== undefined) uniforms.uWarpStrength.value = props.warpStrength;
  if (props.warpScale !== undefined) uniforms.uWarpScale.value = props.warpScale;
  if (props.speed !== undefined) uniforms.uSpeed.value = props.speed;
  if (props.pointerInfluence !== undefined)
    uniforms.uPointerInfluence.value = props.pointerInfluence;
  if (props.pointerStrength !== undefined) uniforms.uPointerStrength.value = props.pointerStrength;
  if (props.refraction !== undefined) uniforms.uRefraction.value = props.refraction;
  if (props.ripple !== undefined) uniforms.uRipple.value = props.ripple ? 1 : 0;
};
...
export const WarpText: React.FC<WarpTextProps> = ({
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const ptr = program.uniforms.uPointer.value as Float32Array;
      ptr[0] = pointer.x;
      ptr[1] = pointer.y;
    ...
    syncUniforms(program as unknown as ProgramWithUniforms, propsRef.current);
    contextRef.current = { program: program as unknown as ProgramWithUniforms, rasterize };
    resize();
    raf = requestAnimationFrame(loop);
