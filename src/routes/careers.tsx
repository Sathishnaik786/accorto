...
import { Heart, Globe2, GraduationCap, Coffee, ShieldCheck, Sparkles, ArrowRight, MapPin } from "lucide-react";

export const Route = createFileRoute("/careers")({
  head: () => ({
    meta: [
      { title: "Careers — Accorto Technologies" },
      { name: "description", content: "Join Accorto. Build category-defining enterprise programs alongside the best in Oracle, SAP, AI and Cloud." },
...
function Careers() {
  return (
    <>
      <PageHero
        tag="Careers"
        title={<>Build the <span className="text-gradient">next era of enterprise tech</span>.</>}
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-16 grid md:grid-cols-3 gap-6">
        {[
          { icon: Heart, t: "Engineering-first culture", d: "Senior engineers in every pod. Real ownership from day one." },
          { icon: Globe2, t: "Global mobility", d: "Hubs in Bengaluru, Dubai and New York. Move where you grow." },
          { icon: Sparkles, t: "Compounding learning", d: "$3,000/yr learning budget. Internal academies for Oracle, SAP, AI." },
        ].map((b, i) => (
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-16">
        <SectionHeading tag="Life at Accorto" title={<>Real <span className="text-gradient">benefits</span>, not perks theater.</>} />
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            <div key={b.t} className="glass rounded-2xl p-5 flex items-center gap-3">
              <b.icon className="h-5 w-5 text-brand" /> <span className="text-sm font-medium">{b.t}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-16">
        <SectionHeading tag="Hiring process" title={<>Respectful, <span className="text-gradient">fast</span>, transparent.</>} />
        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-16">
        <SectionHeading tag="Open positions" title={<>We're hiring. <span className="text-gradient">Across continents.</span></>} />
        <div className="mt-10 space-y-3">
          {JOBS.map((j, i) => (
            <Reveal key={j.title} delay={i * 0.03}>
              <Link to="/contact" className="group flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-2xl glass px-6 py-5 hover:bg-white/[0.05] transition-colors">
                <div>
                  <div className="text-xs text-brand uppercase tracking-widest">{j.team}</div>
                  <div className="font-display text-lg font-semibold group-hover:text-brand transition-colors">{j.title}</div>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-sm text-muted-foreground flex items-center gap-1"><MapPin className="h-3.5 w-3.5" /> {j.loc}</span>
