...
export const Route = createFileRoute("/academy/ai-agents")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Build multi-agent graphs. Learn tool parsing, task execution, state management, and memory trees with LangGraph and CrewAI.",
      },
        ...
        <Reveal>
          <div className="glass-strong rounded-3xl p-8 border border-white/10 shadow-lg space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {[
                { title: "Syllabus 1: Agent Basics", desc: "Single-agent loops, reasoning chains, output parsing, and dynamic tool schemas." },
                { title: "Syllabus 2: State & Memory", desc: "Short-term context memory, persistent storage systems, checkpoint managers, and backtracking loops." },
                { title: "Syllabus 3: Multi-Agent Systems", desc: "Hierarchical agent networks, peer-to-peer conversations, shared state databases, and agent communication protocols." },
                { title: "Syllabus 4: Production Agents", desc: "Telemetry logs, trace visualizers (LangSmith), human-in-the-loop triggers, and cost control limits." },
              ].map((item, idx) => (
