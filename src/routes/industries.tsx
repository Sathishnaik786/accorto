...
import {
  GraduationCap, Stethoscope, Factory, ShoppingBag, Landmark, Truck, Building2,
  ArrowRight, AlertCircle, Sparkles, Target,
} from "lucide-react";

export const Route = createFileRoute("/industries")({
  head: () => ({
    meta: [
      { title: "Industries — Accorto Technologies" },
      { name: "description", content: "Industry expertise across Education, Healthcare, Manufacturing, Retail, Finance, Government, and Logistics." },
      { property: "og:title", content: "Industries we serve" },
      { property: "og:description", content: "Specialized solutions for regulated and complex industries." },
...
const INDUSTRIES = [
  { id: "edu", icon: GraduationCap, name: "Education",
    challenges: ["Fragmented student systems", "Hybrid learning at scale", "Outdated campus operations"],
  ...
  { id: "hc", icon: Stethoscope, name: "Healthcare",
  ...
  { id: "mfg", icon: Factory, name: "Manufacturing",
  ...
  { id: "ret", icon: ShoppingBag, name: "Retail",
  ...
  { id: "fin", icon: Landmark, name: "Finance",
  ...
  { id: "gov", icon: Building2, name: "Government",
  ...
  { id: "log", icon: Truck, name: "Logistics",
...
function Industries() {
  ...
  return (
    <>
      <PageHero
        tag="Industries"
        title={<>Domain depth across <span className="text-gradient">regulated industries</span>.</>}
            ...
            return (
              <button
                ...
                className={`group flex items-center gap-2 rounded-full px-4 py-2 text-sm transition-all ${
                  isActive ? "bg-gradient-brand text-white shadow-lg shadow-brand/30" : "glass hover:bg-white/10"
                }`}
              >
          ...
          <Reveal key={it.id}>
            <div className="grid lg:grid-cols-12 gap-6">
              <div className="lg:col-span-4">
                <Card title="Challenges" icon={AlertCircle} items={it.challenges} tone="text-rose-400" />
              </div>
              <div className="lg:col-span-4">
                <Card title="Our solutions" icon={Sparkles} items={it.solutions} tone="text-brand" />
              </div>
              <div className="lg:col-span-4">
              ...
              <div className="lg:col-span-12">
                <div className="relative overflow-hidden rounded-3xl glass-strong p-8 md:p-10">
                  ...
                  <div className="text-xs uppercase tracking-widest text-muted-foreground">Case highlight</div>
                  <p className="mt-3 font-display text-2xl md:text-3xl font-semibold max-w-3xl leading-tight">{it.highlight}</p>
                  <Link to="/case-studies" className="mt-6 inline-flex items-center gap-2 rounded-full bg-gradient-brand px-5 py-2.5 text-sm font-medium text-white">
...
function Card({ title, icon: Icon, items, tone }: { title: string; icon: any; items: string[]; tone: string }) {
  return (
    <div className="h-full rounded-3xl glass p-6">
      ...
      <ul className="mt-4 space-y-2.5 text-sm">
        {items.map((b) => <li key={b} className="flex gap-2"><span className={`mt-2 h-1 w-1 rounded-full ${tone.replace("text-", "bg-")} shrink-0`} /> {b}</li>)}
      </ul>
    </div>
  );
}
